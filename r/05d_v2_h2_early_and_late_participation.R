...
observations$forks.today <- observations$forks.1 + observations$forks.2 + observations$forks.3 + observations$forks.4 + observations$forks.5
observations$subscribers.today <- observations$subscribers.1 + observations$subscribers.2 + observations$subscribers.3 + observations$subscribers.4 + observations$subscribers.5

observations$subscribers.1 <- (1 / observations$subscribers.today) * observations$subscribers.1
observations$subscribers.2 <- (1 / observations$subscribers.today) * observations$subscribers.2
observations$subscribers.3 <- (1 / observations$subscribers.today) * observations$subscribers.3
observations$subscribers.4 <- (1 / observations$subscribers.today) * observations$subscribers.4
observations$subscribers.5 <- (1 / observations$subscribers.today) * observations$subscribers.5

observations$forks.1 <- (1 / observations$forks.today) * observations$forks.1
observations$forks.2 <- (1 / observations$forks.today) * observations$forks.2
observations$forks.3 <- (1 / observations$forks.today) * observations$forks.3
observations$forks.4 <- (1 / observations$forks.today) * observations$forks.4
observations$forks.5 <- (1 / observations$forks.today) * observations$forks.5

observations <- subset(observations, observations$forks.today > 0 & observations$subscribers.today > 0)
...
observations$ratio.5 <- observations$code_contributions.ratio.5
...
observations$code_contributions.total <- sum(observations$code_contributions.1 + observations$code_contributions.2 + observations$code_contributions.3 + observations$code_contributions.4 + observations$code_contributions.5)
obs <- observations[, c(
  "forks.today",
  "subscribers.today",
  "code_contributions.total",
  "code_contributions.1",
  "code_contributions.2",
  "code_contributions.3",
  "code_contributions.4",
  "code_contributions.5",
  "ratio.1",
  "ratio.2",
  "ratio.3",
  "ratio.4",
...
toFile(content, file = 'hypotheses/h2/h2_glm_top_project_ratio.tex')
...
toFile(content, file = 'hypotheses/h2/h2_glm_top_project_subscribers.tex')
...
content <- stargazer(linear.1, linear.2, linear.3, linear.4, linear.5, linear.6, type=textFormat, float = F)
toFile(content, file = 'hypotheses/h2/h2_glm_top_project_forks.tex')
...
content <- stargazer(linear.1, linear.2, linear.3, linear.4, type=textFormat, float = F)
toFile(content, file = 'hypotheses/h2/h2_glm_top_project_forks_time_periods.tex')

linear.1 <- glm(formula = is_top_project ~ age + ratio.1_2, family=binomial(link='logit'), data = observations )
linear.2 <- glm(formula = is_top_project ~ age + ratio.2_3, family=binomial(link='logit'), data = observations )
linear.3 <- glm(formula = is_top_project ~ age + ratio.3_4, family=binomial(link='logit'), data = observations )
linear.4 <- glm(formula = is_top_project ~ age + ratio.4_5, family=binomial(link='logit'), data = observations )

content <- stargazer(linear.1, linear.2, linear.3, linear.4, type=textFormat, float = F)
toFile(content, file = 'hypotheses/h2/h2_glm_top_project_ratio_period.tex')
...
toFile(content, file = 'hypotheses/h2/h2_glm_forks_ratio_delayed.tex')
...
content <- stargazer(linear.1, linear.2, linear.3, linear.4, linear.5, linear.6, linear.7, linear.8, linear.9, linear.10, linear.11, type=textFormat, float = F)
toFile(content, file = 'hypotheses/h2/h2_glm_subscribers_ratio_delayed.tex')
...
toFile(content, file = 'hypotheses/h2/h2_glm_forks_ratio.tex')
content <- stargazer(linear.9, linear.10, linear.11, linear.12, linear.13, linear.14, type=textFormat, float = F)
toFile(content, file = 'hypotheses/h2/h2_glm_forks_ratio_delay_time_period.tex')
...
content <- stargazer(linear.1, linear.2, linear.3, linear.4, type=textFormat, float = F)
toFile(content, file = 'hypotheses/h2/h2_glm_forks_today_ratio_period.tex')
...
content <- stargazer(linear.1, linear.2, linear.3, linear.4, type=textFormat, float = F)
toFile(content, file = 'hypotheses/h2/h2_glm_subscribers_today_ratio_period.tex')
...
content <- stargazer(linear.1, linear.2, linear.3, linear.4, type=textFormat, float = F)
toFile(content, file = 'hypotheses/h2/h2_glm_forks_today_ratio.tex')
...
content <- stargazer(linear.1, linear.2, linear.3, linear.4, type=textFormat, float = F)
toFile(content, file = 'hypotheses/h2/h2_glm_subscribers_today_ratio.tex')
